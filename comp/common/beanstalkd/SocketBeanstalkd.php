...
define('SYSTEM_IS_WIN', strtoupper(substr(PHP_OS, 0, 3)) === 'WIN');

class SocketBeanstalkd
{
     ...
     */
    protected function _read($length = null, $timeout = 1)
    {
        if (!$this->connected && !$this->connect()) {
            return false;
        }
        ...
        if ($length) {
            ...
            //if(SYSTEM_IS_WIN){
            $packet = rtrim(fgets($this->_connection, 16384), "\r\n");
            //}else{
    ...
    public function reserve($timeout = null)
    {
        ...
        $status = strtok($this->_read(null, $timeout + 1), ' ');
