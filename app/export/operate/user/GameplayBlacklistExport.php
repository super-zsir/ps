...
class GameplayBlacklistExport
{
    public static function export($filePathName, $filterParams): bool
    {
        $headings = self::getHeader($filterParams['guid'],$filterParams['lang']);

        $service = new GameplayBlacklistService();
        $bigAreaList = XsBigarea::getBigAreaCnNameById();
        $status = XsUidGameBlackList::$statusMap;
        return Excel::export()
            ->chunk(function (int $page) use ($filterParams, $service, $bigAreaList, $status) {
                // 每次获取1000条数据导入
                $pageSize = 1000;
