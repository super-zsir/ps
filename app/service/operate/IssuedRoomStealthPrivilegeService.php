...
use Imee\Models\Xs\XsIssuedRoomStealthPrivilegeLog;
use Imee\Models\Xs\XsUserProfile;
use Imee\Service\Helper;
use Imee\Models\Xsst\XsstIssuedRoomStealthPrivilege;
use Imee\Comp\Operate\Auth\Models\Cms\CmsUser;
use Imee\Models\Rpc\PsRpc;
use Imee\Service\StatusService;

class IssuedRoomStealthPrivilegeService
{
    /**
     * 创建下发记录
     */
    public function create($params)
    {
        ...
        $now = time();
        // 1. 主表插入
        [$success, $taskId] = XsstIssuedRoomStealthPrivilege::add([
            'num' => count($uids),
            'days' => $days,
            'remark' => $remark,
            'create_time' => $now,
            'admin_id' => $admin_id,
            'state' => XsstIssuedRoomStealthPrivilege::STATE_PENDING,
            'send_time' => 0,
        ]);
        if (!$success) {
        ...
        [$resp, $code] = $rpc->call(PsRpc::API_ISSUED_ROOM_STEALTH_PRIVILEGE, ['json' => $rpcData]);
        $isSuccess = $code == 200
            && is_array($resp)
        ...
        if (!$isSuccess) {
            ...
            XsstIssuedRoomStealthPrivilege::edit($taskId, [
                'state' => XsstIssuedRoomStealthPrivilege::STATE_FAIL
            ]);
            throw new ApiException(ApiException::MSG_ERROR, $errMsg);
        }
        XsstIssuedRoomStealthPrivilege::edit($taskId, [
            'state' => XsstIssuedRoomStealthPrivilege::STATE_SUCCESS,
            'send_time' => time(),
        ]);
    ...
    public function checkCreate($params)
    {
        ...
        $logs = XsIssuedRoomStealthPrivilegeLog::getListByWhere([
            ['create_time', '>=', $startTime],
            ['uid', 'in', $uids]
        ], 'uid,days,remark,create_time', 'uid asc');
        if ($logs) {
            $userMap = XsUserProfile::getUserProfileBatch($uids);
            $adminName = Helper::getAdminName($params['admin_id'] ?? 0);
            $filePath = '/tmp/room_stealth_privilege_log_' . uniqid() . '.csv';
    ...
    public function getList($params)
    {
        ...
        if (!empty($params['uid'])) {
            $taskIds = XsIssuedRoomStealthPrivilegeLog::getListByWhere([
                ['uid', '=', $params['uid']]
            ], 'task_id');
        ...
        $result = XsstIssuedRoomStealthPrivilege::getListAndTotal($cond, '*', 'id desc', $params['page'] ?? 1, $params['limit'] ?? 20);
        // 格式化时间、查操作人
        ...
        foreach ($result['data'] as &$row) {
            ...
            $row['display_state'] = XsstIssuedRoomStealthPrivilege::$stateMap[$row['state']] ?? '';
        }
        return $result;
    }
    ...
    public function getLogList($params)
    {
        ...
        $list = XsIssuedRoomStealthPrivilegeLog::getListAndTotal($cond, '*', 'id desc', $params['page'] ?? 1, $params['limit'] ?? 20);
        // 查找任务表和操作人
        $taskIds = array_column($list['data'], 'task_id');
        $tasks = $taskIds ? XsstIssuedRoomStealthPrivilege::getListByWhere([
            ['id', 'in', $taskIds]
        ], 'id,admin_id') : [];
    ...
    public function exportLogList($params)
    {
        ...
        $file = '/tmp/issued_room_stealth_privilege_log_' . uniqid() . '.csv';
    ...
    {
        return StatusService::formatMap(XsstIssuedRoomStealthPrivilege::$stateMap);
    }
