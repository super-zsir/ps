...
use Imee\Models\Xs\XsUserRankAnonymousLog;
use Imee\Models\Xs\XsUserProfile;
use Imee\Service\Helper;
use Imee\Models\Xsst\XsstIssuedRankAnonymousPrivilege;
use Imee\Comp\Operate\Auth\Models\Cms\CmsUser;
use Imee\Models\Rpc\PsRpc;
use Imee\Service\StatusService;

class IssuedRankAnonymousPrivilegeService
{
    /**
     * 创建下发记录
     */
    public function create($params)
    {
        ...
        $now = time();
        // 1. 主表插入
        [$success, $taskId] = XsstIssuedRankAnonymousPrivilege::add([
            'num' => count($uids),
            'days' => $days,
            'remark' => $remark,
            'create_time' => $now,
            'admin_id' => $admin_id,
            'state' => XsstIssuedRankAnonymousPrivilege::STATE_PENDING,
            'send_time' => 0,
        ]);
        if (!$success) {
        ...
        [$resp, $code] = $rpc->call(PsRpc::API_SEND_RANK_ANONYMOUS_PRIVILEGE, ['json' => $rpcData]);
        $isSuccess = $code == 200
            && is_array($resp)
        ...
        if (!$isSuccess) {
            ...
            XsstIssuedRankAnonymousPrivilege::edit($taskId, [
                'state' => XsstIssuedRankAnonymousPrivilege::STATE_FAIL
            ]);
            throw new ApiException(ApiException::MSG_ERROR, $errMsg);
        }
        XsstIssuedRankAnonymousPrivilege::edit($taskId, [
            'state' => XsstIssuedRankAnonymousPrivilege::STATE_SUCCESS,
            'send_time' => time(),
        ]);
    ...
    public function checkCreate($params)
    {
        ...
        $logs = XsUserRankAnonymousLog::getListByWhere([
        ...
        if ($logs) {
            ...
            $filePath = '/tmp/rank_anonymous_privilege_log_' . uniqid() . '.csv';
    ...
    public function getList($params)
    {
        ...
        if (!empty($params['uid'])) {
            $taskIds = XsUserRankAnonymousLog::getListByWhere([
                ['uid', '=', $params['uid']]
            ], 'task_id');
        ...
        $result = XsstIssuedRankAnonymousPrivilege::getListAndTotal($cond, '*', 'id desc', $params['page'] ?? 1, $params['limit'] ?? 20);
        // 格式化时间、查操作人
        ...
        foreach ($result['data'] as &$row) {
            ...
            $row['display_state'] = XsstIssuedRankAnonymousPrivilege::$stateMap[$row['state']] ?? '';
        }
        return $result;
    }
    ...
    public function getLogList($params)
    {
        ...
        $list = XsUserRankAnonymousLog::getListAndTotal($cond, '*', 'id desc', $params['page'] ?? 1, $params['limit'] ?? 20);
        // 查找任务表和操作人
        $taskIds = array_column($list['data'], 'task_id');
        $tasks = $taskIds ? XsstIssuedRankAnonymousPrivilege::getListByWhere([
            ['id', 'in', $taskIds]
        ], 'id,admin_id') : [];
    ...
    public function exportLogList($params)
    {
        ...
        $file = '/tmp/issued_rank_anonymous_privilege_log_' . uniqid() . '.csv';
    ...
    {
        return StatusService::formatMap(XsstIssuedRankAnonymousPrivilege::$stateMap);
    }
