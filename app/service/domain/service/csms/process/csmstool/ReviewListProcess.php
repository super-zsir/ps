...
class ReviewListProcess extends NormalListAbstract
{
    ...
    public function __construct(PageContext $context)
    {
        ...
        $this->masterClass = CsmsReviewLog::class;
        $this->query = CsmsReviewLog::query();
    }
    ...
    public function buildWhere()
    {
        ...
        if(is_numeric($this->context->state)){
            $this->where['condition'][] = "state = :state:";
            $this->where['bind']['state'] = intval($this->context->state);
        }
    ...
    public function formatList($items)
    {
        ...
        if($items){
            ...
            foreach ($items as &$item){
                ...
                $item['state_name'] = $item['state'] ? '成功' : '失败';
            }
        }
        return $items;
    }
